...
template <typename T>
cl_mem easyBufferCreate(const T *data, size_t size, cl_mem_flags flags = CL_MEM_READ_WRITE)
{
    InitializeOpenCL();
...
template <typename T>
void easyBufferRead(cl_mem mem, T *data, size_t size, cl_mem_flags flags = CL_MEM_READ_WRITE)
{
    InitializeOpenCL();

    cl_int err;
    err = clEnqueueReadBuffer(OpenClData.queue, mem, true, 0, size, (void *)data, 0, nullptr, nullptr);
    if (err != CL_SUCCESS)
        EXIT_FAIL("Unable to memcpy buffer");
